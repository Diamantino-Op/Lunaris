...
function createKeywordItem(label, detail, documentation) {
    return {
        label,
...
function createSnippetItem(label, detail, snippet, documentation) {
    return {
        label,
...
function lineText(document, line) {
    return document.getText({
        ...
        end: node_1.Position.create(line + 1, 0),
    }).replace(/\r?\n$/, '');
}
const completionItems = [
    createSnippetItem('data', 'Top-level data declaration', 'data ${1:name}: ${2:type} = ${3:value}', 'Creates a top-level data object that the compiler emits into the object file.'),
    createSnippetItem('data section', 'Top-level data declaration with an explicit section', 'data ${1:name}: ${2:type} section "${3:.limine_requests}" = ${4:value}', 'Places the data object into a named ELF section.'),
    createSnippetItem('packed struct', 'Packed structure declaration', 'packed struct ${1:Name}\n    ${2:field}: ${3:type};\nend', 'Declares a packed structure without padding between fields.'),
    createSnippetItem('struct', 'Structure declaration', 'struct ${1:Name}\n    ${2:field}: ${3:type};\nend', 'Declares a structure type.'),
    createSnippetItem('function', 'Function declaration', 'function ${1:name}(${2:args})\n    ${3:return value}\nend', 'Declares a function body.'),
    createSnippetItem('extern asm', 'External assembly declaration', 'asm function ${1:name}(${2:args}): ${3:type} = ${4:symbol};', 'Declares a symbol that is implemented outside the Lua source.'),
    createSnippetItem('if', 'Conditional block', 'if ${1:condition} then\n    ${2}\nend', 'Starts an if block.'),
    createSnippetItem('while', 'Loop block', 'while ${1:condition} do\n    ${2}\nend', 'Starts a while loop.'),
    createSnippetItem('return', 'Return statement', 'return ${1:value}', 'Returns from the current function.'),
...
function currentPrefix(document, position) {
...
function filteredCompletions(prefix) {
    if (!prefix) {
...
function semanticTokensForDocument(document) {
    ...
    const lineTokens = [];
    function pushMatch(lineNumber, lineText, pattern, tokenType, captureGroup = 0) {
        pattern.lastIndex = 0;
...
function diagnostic(range, message) {
    return {
        ...
        range: {
            start: node_1.Position.create(range.startLine, range.startCharacter),
            end: node_1.Position.create(range.endLine ?? range.startLine, range.endCharacter ?? range.startCharacter + 1),
        },
        message,
        source: 'lunaris-lua',
    };
...
function validateDataLine(line, lineNumber, diagnostics) {
...
function validateStructHeader(line, lineNumber, diagnostics) {
...
function validateFunctionHeader(line, lineNumber, diagnostics) {
...
function validateAsmFunction(line, lineNumber, diagnostics) {
...
function validateTextDocument(document) {
    ...
    while (blocks.length > 0) {
        ...
        diagnostics.push(diagnostic({ startLine: block.line, startCharacter: block.character }, "block opened here is missing 'end'"));
    }
...
connection.onRequest(vscode_languageserver_protocol_1.SemanticTokensRequest.type, (params) => {
...
connection.onCompletion((params) => {
    ...
    return node_1.CompletionList.create(filteredCompletions(currentPrefix(document, params.position)), false);
});
